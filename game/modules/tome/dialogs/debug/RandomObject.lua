...
Filters and data are interpreted by either game.zone:checkFilter or game.state:generateRandart respectively,
within the _G environment (used by the Lua Console) using the current zone's #LIGHT_GREEN#object_list#LAST#.  Press #GOLD#'F1'#LAST# for help.
Mouse over controls for a preview of the generated object. (Press #GOLD#'L'#LAST# to lua inspect.)
...
The #LIGHT_GREEN#Random Filter#LAST# controls random generation of a normal object.]])}
	...
	local random_make = self.newButton{text="Create", _object_field="_random_object",
		fct=function()
			CreateItem.acceptObject(self, _M._random_object)
		end,
		help_display = "filter_help"
	}
	...
	local random_clear = self.newButton{text="Clear", _object_field="_random_object",
		fct=function()
			if _M._random_object then
				_M._random_object:removed()
	...
	local random_txt = Textzone.new{auto_width=true, auto_height=true, no_color_bleed=true, font=self.font,
	text=object_text("Random Object", _M._random_object), can_focus=true}
	random_txt.refresh = function(self, object)
		self.text = object_text("Random Object", object)
		self:generate()
	end
	random_txt.on_focus = function() _M.tooltip(_M._random_object) _M.help_display = "filter_help" end
	self.random_txt = random_txt
	
	local rf_box = _M.newTextbox{title="#LIGHT_GREEN#Random Filter:#LAST# ", text=_M._random_filter or "{}", chars=120, max_len=500,
		fct=function(text)
	...
	text=([[The #LIGHT_BLUE#Base Filter#LAST# is used when generating a base object to use as the starting point for building a Randart.]]):format()}
	...
	text = object_text("Base Object", _M._base_object), can_focus=true}
	base_txt.refresh = function(self, object)
		self.text = object_text("Base Object", _M._base_object)
		self:generate()
	end
	...
	local base_make = self.newButton{text="Create", _object_field="_base_object",
		fct=function()
			CreateItem.acceptObject(self, _M._base_object)
		end,
		help_display = "filter_help"
	}
	...
	local base_clear = self.newButton{text="Clear", _object_field="_base_object",
		fct=function()
			if _M._base_object then
				_M._base_object:removed()
				_M._base_object = nil
			end
			self.base_txt:refresh(_M._base_object)
	...
	text=([[#ORANGE#Randart Data#LAST# contains parameters used to generate a Randart. (see game.state:generateRandart for a list of parameters).
If a base object is not provided, it will be generated randomly.]]):format()}
	...
	local randart_make = self.newButton{text="Create", _object_field="_randart",
		fct=function()
			self.randart_txt:refresh(_M._randart)
			CreateItem.acceptObject(self, _M._randart)
		end,
		help_display = "data_help"
	}
	...
	local randart_data_box = _M.newTextbox{title="#ORANGE#Randart Data:#LAST# ", text=_M._randart_data or "{}", chars=120, max_len=500,
		fct=function(text)
	...
	local randart_txt = Textzone.new{auto_width=true, auto_height=true, no_color_bleed=true, font=self.font,
	text=object_text("Randart", _M._randart), can_focus=true}
	randart_txt.refresh = function(self, object)
		self.text = object_text("Randart", object)
		self:generate()
	end
...
-- display the tooltip for an object
function _M.tooltip(obj)
	if obj then
		local plr = game.player
		game:tooltipDisplayAtMap(game.w, game.h, obj:getDesc({do_color=true}, nil, true, plr), nil, true)
	else
		game:tooltipDisplayAtMap(game.w, game.h, "#GREY#No Object to Display#LAST#", nil, true)
	end
end
	...
	self.key:addCommands{
		_l = function() -- lua inspect object
			...
			if obj then
				...
				game.log("#LIGHT_BLUE#No object to Lua inspect")
			end
		end,
		}
	return self
...
--- Generate an object by a filter
-- This (mostly) matches resolvers.equip with support for filer.base_list
function _M:generateByFilter(filter)
	local o
print("custom makeEntity function making random base object with filter", filter) table.print(filter) -- debugging
	if filter and filter.base_list then game.log("#GOLD#WARNING:#LIGHT_BLUE# filter.base_list not supported by all resolvers") end
	if filter and filter.defined then
		if filter.base_list then -- note this only applies to resolvers.inventory normally
			...
			local forced
			o, forced = game.zone:makeEntityByName(game.level, "object", filter.defined)
			if forced then
...
function _M:generateRandom()
	local ok, filter
--	game.log("#LIGHT_BLUE# Generating random object using filter: %s", _M._random_filter)
	local fx = loadstring("return "..tostring(_M._random_filter))
	self.random_filter_function=fx
	if fx then
		setfenv(fx, _G)
		ok, filter = pcall(fx)
	end
	if not ok or filter and type(filter) ~= "table" then
		game.log("#LIGHT_BLUE#Bad filter for base object: %s", _M._random_filter)
		return
	end
	local o
	...
	if ok then
		if o then
			if _M._random_object then _M._random_object:removed() end
...
function _M:generateBase()
	...
	if fx then
		setfenv(fx, _G)
		ok, filter = pcall(fx)
		--print("Called filter for base:", filter) table.print(filter, "\t_cf_")
	end
	if not ok or filter and type(filter) ~= "table" then
		game.log("#LIGHT_BLUE#Bad filter for base object: %s", _M._base_filter)
		return
	end
	local o
	ok, o = pcall(_M.generateByFilter, self, filter)
	if ok then
		if o then
			if _M._base_object then _M._base_object:removed() end
			_M._base_object = o
			o:identify(true)
		else
...
function _M:generateRandart()
	...
	--game.log("#LIGHT_BLUE#Creating Randart from %s using data: %s", base and base.name or "random base", _M._randart_data)
	local ok, data
	local fx = loadstring("return "..tostring(_M._randart_data))
	self.randart_data_function = fx
	if fx then
		setfenv(fx, _G)
		ok, data = pcall(fx)
	end
	if not ok or data and type(data) ~= "table" then
		game.log("#LIGHT_BLUE#Bad data for Randart generation: %s", _M._randart_data)
		return
	end
	data = data or {}
	...
	if ok then
		if o then
			...
			if _M._randart then _M._randart:removed() end
