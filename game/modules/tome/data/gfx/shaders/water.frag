...
void main() 
{
	vec2 wsize = vec2(64.0, 64.0);
	vec2 p = (vec2(gl_FragCoord.x - mapCoord.x, wsize.y - gl_FragCoord.y - mapCoord.y)) / wsize.xy, c1 = p, c2 = p;
	float cc1 = water_col(c1);

	c2.x += wsize.x/water_delta;
	float dx = water_emboss*(cc1-water_col(c2))/water_delta;

	c2.x = p.x;
	c2.y += wsize.y/water_delta;
	...
	gl_FragColor = texture2D(tex, te4_uv) * te4_fragcolor * (alpha);
}
