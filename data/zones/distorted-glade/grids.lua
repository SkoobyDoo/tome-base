...
newEntity{
	...
	change_level_check = function() -- Forbid going back
		if game.player:hasQuest("atof+point-zero") then
			require("engine.ui.Dialog"):yesnoPopup("Temporal Portal", "Are you sure you want to enter? You won't be able to go back.", function(ret)
				if ret then game:changeLevel(1, "atof+point-zero") end
			end)
			return true
		end
		game.log("You don't know where, or rather when, this portal is leading to.")
		return true
	end
}
