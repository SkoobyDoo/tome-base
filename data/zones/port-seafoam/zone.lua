...
return {
	...
	post_process = function(level)
		game.state:makeAmbientSounds(level, {
			jungle1={ chance=250, volume_mod=0.6, pitch=0.6, random_pos={rad=10}, files={"ambient/jungle/jungle2"}},
			jungle2={ chance=250, volume_mod=1, pitch=1, random_pos={rad=10}, files={"ambient/jungle/jungle2"}},
			jungle3={ chance=250, volume_mod=1.6, pitch=1.4, random_pos={rad=10}, files={"ambient/jungle/jungle2"}},
		})
	end,
	...
	foreground = function(level, x, y, nb_keyframes)
		...
		if not level.bird then
			if nb_keyframes > 0 and rng.chance(500 / nb_keyframes) then
				...
				level.bird = require("engine.Particles").new("eagle", 1, {x=0, y=y, dir=dir, dirv=dirv, size=size, life=800, vel=7, image="particles_images/birds_tropical_03"})
			end
		else
