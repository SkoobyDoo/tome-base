			...
			for (int gi = 1; gi <= nb_generators; gi++) {
				...
				switch (g_id) {
					...
					case GeneratorsList::DiskPosGenerator:
						g = new DiskPosGenerator(lua_float(L, -1, "min_angle", 0), lua_float(L, -1, "max_angle", M_PI*2), lua_float(L, -1, "radius", 100));
						g->basePos(lua_float(L, -1, "sx", 0), lua_float(L, -1, "sy", 0));
						break;
					case GeneratorsList::CirclePosGenerator:
						g = new CirclePosGenerator(lua_float(L, -1, "min_angle", 0), lua_float(L, -1, "max_angle", M_PI*2), lua_float(L, -1, "radius", 100), lua_float(L, -1, "width", 10));
						g->basePos(lua_float(L, -1, "sx", 0), lua_float(L, -1, "sy", 0));
						break;
