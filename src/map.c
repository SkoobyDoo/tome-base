...
static int map_objects_display(lua_State *L)
{
	...
	CHECKGL(tglBindFramebuffer(GL_FRAMEBUFFER, fbo));
	...
	CHECKGL(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 0, 0));
	CHECKGL(tglBindFramebuffer(GL_FRAMEBUFFER, gl_c_fbo));
	// Restore viewport
	CHECKGL(glPopAttrib());
