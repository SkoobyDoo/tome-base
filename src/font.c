...
bool font_add_atlas(font_type *f, int32_t c, font_style style) {
	...
	SDL_Surface *txt = TTF_RenderGlyph_Solid(f->font, c, color);
...
void font_update_atlas(font_type *f) {
	if (f->atlas_changed) {
		build_sdm_ex(f->atlas->pixels, f->atlas_w, f->atlas_h, f->sdm_atlas->pixels, f->atlas_w, f->atlas_h, 0, FALSE);

		tfglBindTexture(GL_TEXTURE_2D, f->atlas_tex);
		copy_surface_to_texture(f->sdm_atlas);
		f->atlas_changed = FALSE;
	}
}
...
void font_make_atlas(font_type *f, int w, int h) {
	...
	amask = 0xff000000;
#endif
	f->sdm_atlas = SDL_CreateRGBSurface(SDL_SWSURFACE, w, h, 32, rmask, gmask, bmask, amask);
	...
	if (!f->atlas_tex) {
		...
		tfglBindTexture(GL_TEXTURE_2D, f->atlas_tex);
		make_texture_for_surface(f->sdm_atlas, NULL, NULL, false);
