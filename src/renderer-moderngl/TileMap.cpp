...
void DORTileMiniMap::redrawMiniMap(bool full_texture_update) {
	...
	if (full_texture_update) glTexImage2D(GL_TEXTURE_2D, 0, 4, info.mdw, info.mdh, 0, GL_RGBA, GL_UNSIGNED_BYTE, mm_data);
	else glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, info.mdw, info.mdh, GL_RGBA, GL_UNSIGNED_BYTE, mm_data);
}
