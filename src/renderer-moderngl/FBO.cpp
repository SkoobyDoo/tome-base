	...
	: TargetSpecialMode(t), vbo(VBOMode::STATIC)
{
	...
	target->makeFramebuffer(target->w, target->h, 1, true, false, &fbo_plain);
	target->makeFramebuffer(target->w, target->h, 1, true, false, &fbo_bloom);
	target->makeFramebuffer(target->w, target->h, 1, true, false, &fbo_hblur);
	target->makeFramebuffer(target->w, target->h, 1, true, false, &fbo_vblur);
...
void TargetBloom2::renderMode() {
	...
	for (int i = 0; i < blur_passes; i++) {
		target->useFramebuffer(fbo_blur);
		vbo.setTexture((i == 0) ? target->fbo.textures[1].texture : fbo_blur_prev->textures[0].texture);
	...
	// Merge
	target->useFramebuffer(&fbo_plain);
	vbo.setTexture(target->fbo.textures[0].texture, 1); // Use plain rendering
	vbo.setTexture(target->fbo.textures[1].texture, 0); // Use plain rendering
	vbo.setTexture(fbo_blur_prev->textures[0].texture, 0); // Use the last of the blurs
	vbo.setShader(combine);
	vbo.toScreen(model);

	// Draw back into the normal FBO
	target->useFramebuffer(&target->fbo);
	vbo.setTexture(fbo_plain.textures[0].texture, 0); // Use plain rendering
	vbo.setShader(NULL);
	vbo.toScreen(model);
